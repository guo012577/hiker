...
function updateRecordsBottom(records) {
    ...
    let myRecordDetail = new JavaAdapter(com.lxj.xpopup.core.BottomPopupView, {
        ...
        onCreate() {
            ...
            let statusBarHeight = MyStatusBarUtil.getStatusBarHeight(getActivityContext())
                + DisplayUtil.dpToPx(getActivityContext(), 86);
            ...
            let adapter = new UpdateRecordsAdapter(getActivityContext(), rules);
            recyclerView.setLayoutManager(new GridLayoutManager(getActivityContext(), 1));
            recyclerView.setAdapter(adapter);
        }
    }, getActivityContext());

    // ✅ 使用 XPopup.Builder 而不是自定义 builderXPopup
    let pop = new XPopup.Builder(getActivityContext())
        .asCustom(myRecordDetail)
        .show();

    return myRecordDetail;  // ✅ 返回 PopupView 本身而非 Builder
}

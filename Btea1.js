...
if (!fetch(version.requirefile)) {
    writeFile(version.requirefile, JSON.stringify(defaultBteaConfigs));
}
...
function funcLoadpage(d, obj) {
    ...
    function ChineseToNumber(chinese_number) { //中文转数字
        ...
        if (len === 0)
            return -1;
        if (len === 1)
            return (ChineseMap[chinese_number] <= 10) ? ChineseMap[chinese_number] : -1;
        let summary = 0;
        ...
        var menuIcons = {
            ...
            "最近": {
                ...
                url: $('#noLoading#').lazyRule(() => {
                    ...
                    return $(sel_rules, 2, '请选一个最近使用的小程序').select(() => {
                        return "hiker://home@" + input
                    })
                }),
            }
        };
        menuIcons = Object.keys(menuIcons).filter(function (key) {
            return menuIcons[key] !== '';
        }).reduce(function (obj, key) {
            obj[key] = menuIcons[key];
            return obj;
        }, {});
        var menuNames = [];
                        ...
                        if (!!w[0] || !!w[1]) {
                            ...
                            d.push({
                                ...
                                url: $('#noLoading#').lazyRule((w_Expanded, w_Folded) => {
                                    ...
                                    updateItem('wsfold_button', {
                                        title: w_fold == '0' ? w_Expanded : w_Folded
                                    });
                    ...
                    if (typeof(zd[0]) !== 'undefined') {
                        ...
                        d.push({
                            ...
                            url: $('#noLoading#').lazyRule((Expanded, Folded) => {
                                ...
                                updateItem('zd1_button', {
                                    title: fold1 == '0' ? Expanded : Folded
                                });
                    ...
                    if (typeof(zd[2]) !== 'undefined') {
                        ...
                        d.push({
                            ...
                            url: $('#noLoading#').lazyRule((Expanded, Folded) => {
                                ...
                                updateItem('zd2_button', {
                                    title: fold2 == '0' ? Expanded : Folded
                                });
                    ...
                    if (typeof(zd[4]) !== 'undefined') {
                        ...
                        d.push({
                            ...
                            url: $('#noLoading#').lazyRule((Expanded, Folded) => {
                                ...
                                updateItem('zd3_button', {
                                    title: fold3 == '0' ? Expanded : Folded
                                });
...
function menusetting(d) {
    ...
    if (foldDIY === '0') {}
        ...
        d.push({
            title: '<span style="color:#fa8c35">' + '周表样式:</span>' + getItem('周表样式', ''),
            col_type: 'text_icon',
            img: ZbImage,
        ...
        d.push({
            title: '<span style="color:#fa8c35">' + '折叠样式:</span>' + getItem('首页折叠样式', ''),
            col_type: 'text_icon',
            img: ZdImage,
        ...
        d.push({
            title: '<span style="color:#fa8c35">' + '推荐样式:</span>' + getItem('首页推荐样式', ''),
            col_type: 'text_icon',
            //extra: {lineVisible: false},
            img: TjImage,
        ...
        d.push({
            title: '<span style="color:#fa8c35">' + '片库样式:</span>' + getItem('片库样式', ''),
            col_type: 'text_icon',
            //extra: {lineVisible: false},
            img: PkImage,
        ...
        d.push({
            title: '<span style="color:#fa8c35">' + '热榜样式:</span>' + getItem('热榜样式', ''),
            col_type: 'text_icon',
            img: TjImage,
        ...
        d.push({
            title: '<span style="color:#fa8c35">' + 'tab样式:</span>' + getItem('tab样式', ''),
            col_type: 'text_icon',
            //extra: {lineVisible: false},
            img: TabImage,
        ...
        d.push({
            title: '<span style="color:#fa8c35">' + '选集样式:</span>' + getItem('选集样式', ''),
            col_type: 'text_icon',
            //extra: {lineVisible: false},
            img: XjImage,
    ...
    d.push({
        title: '<span style="color:#3eede7">' + '通免:</span>' + getItem("通免", ""),
        col_type: "text_icon",
        img: TmImage,
    ...
    d.push({
        ...
        url: $('恢复所有自定义配置项为默认，是否继续?').confirm(() => {
            ...
            setItem('首页折叠样式', 'movie_3_marquee');
            setItem('首页推荐样式', 'movie_3_marquee');
...
var 二级 = {
    A: function (p, lazy, dp) {
        ...
        if (getItem("通免", "通免") == "道长通免") {
            def_lazy = config.道长通免
        } else if (getItem("通免", "通免") == "香免") {
            def_lazy = config.香免
        } else {
            def_lazy = config.通免
        };
        ...
        const getStyle = (lists) => {
            ...
            const mapping = styleMappings.find(({
                        min,
                        max
                    }) => lists.length >= min && lists.length <= max) || {
                style: 'flex_button'
            };
    ...
    Mycol.map(it => {
        try {
            ...
            if (restitle == group || restitle == "全部♨️" || restitle == '') {
                ...
                //log('txt' + txt);
                let url = mode == "剪贴" ? $(`#noLoading#`).lazyRule((txt) => {
                    refreshPage(false);
                    return txt;

                }, txt) : it.cUrl + `@rule=js:` + fixSemicolon(lr.preRule) + fixSemicolon(params.find_rule);
                let extra = mode == "剪贴" ? {
                    img: it.picUrl,
                }
                 : extraObj;
    ...
    d.push({
        ...
        url: $('显示名@@小程序名@@图片链接', '根据提示输入就好了\n小程序名为空则为海阔搜索').input(() => {
            ...
            if (input.length != 3 || input[0] === "")
                return "toast://格式不对，按格式输入!";
            if (quickSearchConfigs.order.indexOf(input[0]) == -1)
                quickSearchConfigs.order.push(input[0]);
    ...
    d.push({
        ...
        url: $(quickSearchConfigs.order, 2).select(() => {
            ...
            return $(dtext, '请修改').input((config, raw) => {
                ...
                if (config.quickSearchConfigs.order.indexOf(input[0]) == -1)
                    config.quickSearchConfigs.order.push(input[0]);
        ...
        function 通免() {
            ...
            var lazy = $('').lazyRule((MY_HOME) => {
                ...
                function playerConfigAes(jsurl, player) {
                    ...
                    var configRes = request(jx + jsurl, {
                        headers: {
                            'Referer': MY_URL
                        }
                    });
                ...
                try {
                    ...
                    if (/\.m3u8|\.mp4|\.flv/.test(input)) {
                        return clearM3u8Ad(input + '#isM3u8#', {
                            headers: {}
                        });
                    }
                    ...
                    if (!player)
                        throw new Error('player_data 解析失败');

                    var url = decryptUrl(player);

                    // 4. 解密后是直链
                    if (/\.m3u8|\.mp4|\.flv/.test(url)) {
                        return clearM3u8Ad(url + '#isM3u8#', {
                            headers: {}
                        });
                    }
